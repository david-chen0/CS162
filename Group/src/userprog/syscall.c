...
int sys_read(int handle, void* udst_, unsigned size) {
  ...
  // // Error if the input FD corresponds to a directory
  struct inode* fileInode = file_get_inode(fd->file);
  if (isDirectory(fileInode)) {
    return -1;
  }
...
// Change the current working directory of the process and return whether operation was successful
bool sys_chdir(const char* dir) {
  // Get the directory to switch to
  struct dir* nextDir = getDirectory(dir);

  // Failed in getting requested directory
  if (nextDir == NULL) {
    return false;
  }
  
  // Successful in getting requested directory
  struct process* curProcess = thread_current()->pcb;
  dir_close(curProcess->cwd);
  curProcess->cwd = nextDir;
  return true;
}

// Create the directory and return whether operation was successful
// Fail if dir already exists or if any directory name in dir(besides the last) does not already exist
bool sys_mkdir(const char* dir) {
  return filesys_create(dir, 0, true);
}

// Reads a directory entry from file descriptor fd and stores the null-terminated filename in name,
// which must be able to support READDIR_MAX_LEN + 1 bytes
// Do not read  . or ..
// Return if operation was sucessful
bool sys_readdir(int fd, char* name) {
  // Get the file's inode
  struct inode* inode = getInode(fd);
...
// Return whether the fd corresponds to a directory
bool sys_isdir(int fd) {
  ...
  return isDirectory(inode);
}

// Returns the inode number of the inode associated with fd, which can be a file or a directory
