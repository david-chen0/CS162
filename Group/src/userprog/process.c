...
// Helper method to look through process's FD table to get the file_descriptor corresponding to the input FD
// Returns NULL if the input FD doesn't match anything in the list
struct file_descriptor* getFD(int fd) {
  // Get the process's FD table
  struct list fdTable = thread_current()->pcb->fds;
  for (struct list_elem* elem = list_head(&fdTable); elem != list_end(&fdTable); elem = list_next(elem)) {
    struct file_descriptor* FD = list_entry(elem, struct file_descriptor, elem);
    if (FD->handle == fd) { // Found our file_descriptor that we were looking for
      return FD;
    }
  }
  return NULL;
...
// Helper method to get the input fd's inode
struct inode* getInode(int fd) {
  // Look through FD table for directory
  struct file_descriptor* FD = getFD(fd);
  if (FD == NULL) {
    return NULL;
  }
